...
_BAD_HOST_PARTS = {"yandex.ru", "ya.ru", "aistudio.yandex.ru", "yabs.yandex.ru", "passport.yandex.ru"}
...
def _odds_lines(text: str, event: Event) -> str:
    ...
    anchors = ("исход", "итоговая победа", "победитель", "тоталы", "форы", "winner", "moneyline", "1 x 2", "победа", "тотал", "фора")
    ...
    for line in interesting:
        ...
        if len(result) >= 60:
            break
    compact = "\n".join(result)[:9000]
    return compact if _ODDS_RE.search(compact) else ""
...
async def _collect_bookmaker(event: Event, bookmaker: str, domain: str, landing_url: str) -> tuple[str, str | None, list[str]]:
    ...
    for query in queries:
        try:
            ...
            errors.append(f"{bookmaker}: search {type(exc).__name__}: {exc}")
            continue
        for url, text in pages:
            ...
            compact = _odds_lines(text, event)
            if compact:
                print(f"ODDS FOUND: {bookmaker} search", flush=True)
...
async def collect_bookmaker_odds(event: Event) -> tuple[dict[str, str], list[str]]:
    """Find one verified line from an official Russian bookmaker."""
    found: dict[str, str] = {}
    errors: list[str] = []
    for bookmaker, domain, landing_url in BOOKMAKERS:
        ...
        if text:
            found[name] = text
            break
    print(f"ODDS RESULT: found={list(found)} errors={len(errors)}", flush=True)
    return found, errors
